...
export default async function MsOutreachPage({
  ...
  const monthCols = getFiscalQuarterMonths(fy, quarter - 1);
  ...
  const [workers, entries, goal] = await Promise.all([
    ...
    prisma.msKpiGoal.findUnique({ where: { period: fyPeriodLabel(fy) } }),
  ]);
  ...
  return (
    <>
      <Header
        title="MS送付状況"
        subtitle={`アウトリーチ送付・アポ獲得KPI｜${fyDisplayLabel(fy)} ${quarter}Q`}
      />
