        ...
        <div className="relative">
          ...
          {open && (
            <>
              ...
              <div className="absolute left-0 top-full z-20 mt-1 grid max-h-[75vh] w-[min(40rem,calc(100vw-2rem))] grid-cols-2 gap-x-1 overflow-y-auto rounded-md border border-zinc-200 bg-white p-1.5 shadow-lg">
                {masters.length === 0 && (
                ...
                {masters.map((m) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full items-center gap-2.5 rounded px-2.5 py-2 text-left text-sm hover:bg-zinc-50 ${checked ? "bg-orange-50" : ""}`}
                    >
                      <span
                        className={`flex h-5 w-5 shrink-0 items-center justify-center rounded border ${checked ? "border-orange-500 bg-orange-500 text-white" : "border-zinc-300"}`}
                      >
                        {checked && <Check className="h-3.5 w-3.5" />}
                      </span>
                      <span className={`truncate rounded px-2 py-0.5 ${c.bg} ${c.text}`} title={m.name}>{m.name}</span>
                    </button>
                  );
                })}
              </div>
